...
def parse_one():
    ...
    def plot(self, X0_rep, Theta0, save_path=None, show=True):
        n_row, n_col = 2, 4
        fig, axis = plt.subplots(n_row, n_col, figsize=(4 * n_col, 4 * n_row))#, sharex=True, sharey=True)
    # set_trace()

        plot_dist(self, X0_rep, Theta0, axis=axis[0],save_path=None, show=False)
    ...
    def plot_dist(self, X0_rep, Theta0, axis=None, save_path=None, show=True):
        if axis is None:
            n_row, n_col = 1, 4
            fig, axis = plt.subplots(n_row, n_col, figsize=(4 * n_col, 4 * n_row))#, sharex=True, sharey=True)

        sample = self.sampler(X0_rep.to(args.device)).numpy()
        ...
        if j % 10 == 0 and j != 0:
            ...
            for kk in range(5):
                Theta1 = [
                    ...
                    np.random.uniform(0, 0.1)]
                X1 = integrate(a0 = [Theta1[0]],
                    b0 = [Theta1[1]],
                    c0 = [Theta1[2]],
                    d0 = [Theta1[3]],
                    x0=100, y0=50,
                    timestep=0.1, np_random=None, seed=1234, n_steps=args.n_steps)
                X1 = X1[:,0:2] #removing time
                X1 = X1.transpose(0, 2, 1)  # Rearrange the dimensions
                X1_rep = torch.from_numpy(np.repeat(X1, repeats=150, axis=0)).float().to(args.device)
                plot_dist(method, X1_rep, Theta1, show=False,  
                      save_path=net_path+f"epoch{BY*(j+1)}_additional{kk}.png")
